...
```
"""
function generator_sif_problems(;
  libsif_folder::AbstractString = libsif_path,
  filter::Function = name->true,
)
  return (
    replace(basename(file), r".SIF$" => "") for file in readdir(libsif_folder, join = true) if
    endswith(file, ".SIF") && filter(replace(basename(file), r".SIF$" => ""))
  )
end
