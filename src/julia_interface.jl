...
function NLPModels.objcons(nlp :: CUTEstModel, x :: Array{Float64,1})
...
function NLPModels.objgrad(nlp :: CUTEstModel, x :: Array{Float64,1})
...
function NLPModels.obj(nlp :: CUTEstModel, x :: Array{Float64,1})
  f = objcons(nlp, x)[1]
  if nlp.meta.ncon > 0
...
function NLPModels.grad(nlp :: CUTEstModel, x :: Array{Float64,1})
  g = Array{Float64}(undef, nlp.meta.nvar)
  return grad!(nlp, x, g)
end

@doc (@doc NLPModels.grad!)
function NLPModels.grad!(nlp :: CUTEstModel, x :: Array{Float64,1}, g :: Array{Float64,1})
  objgrad!(nlp, x, g)
...
function NLPModels.cons(nlp :: CUTEstModel, x :: Array{Float64,1})
  c = Array{Float64}(undef, nlp.meta.ncon)
  cons!(nlp, x, c)
end

@doc (@doc NLPModels.cons!)
function NLPModels.cons!(nlp :: CUTEstModel, x :: Array{Float64,1}, c :: Array{Float64,1})
  objcons!(nlp, x, c)
  nlp.counters.neval_obj -= 1  # does not really count as a objective eval
  return c;
end
...
function NLPModels.jac_coord(nlp :: CUTEstModel, x :: Array{Float64,1})
  c, jrow, jcol, jval = cons_coord(nlp, x)
  nlp.counters.neval_cons -= 1  # does not really count as a constraint eval
...
function NLPModels.jac(nlp :: CUTEstModel, x :: Array{Float64,1})
  c, J = consjac(nlp, x)
  nlp.counters.neval_cons -= 1  # does not really count as a constraint eval
  return J
end
...
function NLPModels.jprod(nlp :: CUTEstModel, x :: Array{Float64,1}, v :: Array{Float64,1})
...
function NLPModels.jtprod(nlp :: CUTEstModel, x :: Array{Float64,1}, v :: Array{Float64,1})
...
function NLPModels.hess(nlp :: CUTEstModel, x :: Array{Float64,1}; y :: Array{Float64,1} = zeros(nlp.meta.ncon), obj_weight :: Float64=1.0)
...
function NLPModels.hprod(nlp :: CUTEstModel, x :: Array{Float64,1}, v :: Array{Float64,1}; y :: Array{Float64,1} = zeros(nlp.meta.ncon), obj_weight :: Float64=1.0)
